...
\centerline{\large \bf Day 11 (Counting Part 3)}
\vspace{\baselineskip}

%\noindent \underline{\hspace{18cm}}

\header{Day 11 (Counting Part 3) } \\
...
\subheader{Pigeonhole Principle: Recap + Example} 
...
\vspace{.25cm}
\textbf{Basic (PH):} If there are $p$ pigeons and $h$ holes with $p > h$, then there is a hole with at least 2 pigeons

	\vspace{.25cm}
	\hspace{1cm} The proof involves contradiction 

\vspace{.75cm}
\textbf{Generalized (GPH):} If there are $p$ pigeons and $h$ holes, then there is a hole with at least $\ds{ \left \lceil \frac{n}{h} \right \rceil }$ pigeons

\vspace{.5cm}
	\hspace{1cm} e.g. If there are 17 pigeons and 5 holes, there must be a hole with at least $\ds{ \left \lceil \frac{17}{5} \right \rceil = \lceil 3.4 \rceil = 4 }$ pigeons
\vspace{.25cm}   
}}
%%% end box
...
\textbf{Ex 1:} Show that if eleven integers are selected from the first twenty positive integers 

\vspace{.25cm}
(a) there must be a pair of these integers with a sum equal to 21. 



%%%
\newpage 
(b) there must be a pair of these integers such that one evenly divides the other. 
...
\textbf{Ex 2:} Prove that in any set of 5 points selected inside a square of side length 2 that there are at least 2 points such that the distance between them is at most $\sqrt{2}$. 
...
\textbf{Ex 3:} Suppose that we need to select a 4-character passcode where each character is a capital English letter. 
...
\textbf{Ex 4:} A certain state's license plates consists of 6 characters which can either be capital English letters or digits. How many possible license plates can be made
...
\textbf{Ex 5:} In the set up of \#1 on the previous page, how many passcodes are there that 
...
\textbf{Ex 6:} In a certain Hogwarts class, there are 8 Gryffindors, 10 Slytherins, 5 Ravenclaws, and 6 Hufflepuffs (Gyffindor, Slytherin, Ravenclaw, and Hufflepuff are \textbf{houses} -- each student belongs to one house). Suppose that a group of students forms a study group. 
...
\textbf{Ex 7:} Suppose that there are 20 people at a party. Prove that there must be at least 2 people at the party with the same number of friends. 
...
\textbf{Ex 8:} There are 51 houses on a street. Each house has an address between 1000 and 1099, inclusive. Show that there are at least two houses whose addresses are consecutive integers. 


\vspace{6cm}


\textbf{Ex 9:} Show that whenever 25 Californians and 25 New Yorkers are seated around a circular table, there is always a person both of whose neighbors are New Yorkers. 
...
\textbf{Ex 10:} Prove that in any set of 5 points selected inside an equilateral triangle of side length 2 that there are at least 2 points such that the distance between them is at most 1. 
